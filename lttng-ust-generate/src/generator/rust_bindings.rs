...
pub(super) fn generate_rust_bindings(
    output_path: &PathBuf,
    providers: &[Provider],
    raw_bindings: &PathBuf,
) -> io::Result<()> {
...
fn generate_instance_call(
    ...
    let args: Vec<String> = class
        ...
        .iter()
        .enumerate()
        .map(|(i, field)| format!("a{}: {}", i, rust_type_for(&field.ctf_type)))
        .collect();
    let args = &args.join(", ");
    let c_args: Vec<String> = class
        .fields
        .iter()
        .enumerate()
        .map(|(i, field)| c_arg_for_field(format!("a{}", i), field))
        .collect();
...
fn rust_type_for(ty: &CTFType) -> String {
    use CTFType::*;
    match *ty {
        Integer(i) | IntegerNoWrite(i) | IntegerHex(i) | IntegerNetwork(i)
        | IntegerNetworkHex(i) => i.rust_type().into(),

        Float(f) | FloatNoWrite(f) => f.rust_type().into(),

        String | StringNoWrite => "::std::ffi::CStr".into(),

        Array(i, l) | ArrayNoWrite(i, l) => format!("&[{}; {}]", i.rust_type(), l),

        ArrayText(_) => "&str".into(),

        Sequence(i) | SequenceNoWrite(i) => format!("&[{}]", i.rust_type()),

        SequenceText | SequenceTextNoWrite => "&str".into(),

        Enum | EnumNoWrite => unimplemented!(),
    }
}
...
fn c_arg_for_field(base_name: String, field: &Field) -> String {
    if let CTFType::SequenceText = field.ctf_type {
        format!(
            "::std::mem::transmute({0}.as_bytes().as_ptr()), {0}.len()",
            base_name
        )
